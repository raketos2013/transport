            ...
            : StepOperation(step, operation, scopeFactory)
{
    ...
    {
        var logger = scopeFactory.ServiceProvider.GetRequiredService<ILogger<Copy>>();
                ...
                else
                {
                    ...
                    if (File.Exists(fileNameDestination))
                    {
                        if (operation == null)
                        {
                            ...
                            {
                                DateTimeLog = DateTime.Now,
                                Text = "Не удалось скопировать файл. Файл уже существует",
                                FileName = fileName,
                                ResultOperation = ResultOperation.E
                            });
                            await _taskLogger.LogFiles(TaskStep, logs);
                        ...
                        {
                            (isOverwriteFile, destFileName) = ExistInDestination(operation, fileName, ref logs);
                        }
                    }
    ...
    {
        var taskLogsAsync = _taskLogService.GetLogsByTaskId(TaskStep.TaskId);
    ...
    private (bool, string) ExistInDestination(OperationCopyEntity operation, string fileName, ref List<FileLog> logs)
    {
