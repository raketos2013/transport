...
{
    public async Task SendRename(TaskStepEntity step, List<AddresseeEntity> addresses, List<(string OldFileName, string NewFileName)> files)
    {
        ...
        try
        {
            ...
            foreach (var (OldFileName, NewFileName) in files)
            {
                message.Append($"<br>{OldFileName.ToString()} переименован в {NewFileName.ToString()}");
            }
    ...
    public async Task SendOffSteps(string taskId, List<AddresseeEntity> addresses, List<int> numberSteps)
    {
        ...
        try
        {
            ...
            message.Append("<br>---------------------------------------------------------------------------------------");
            message.Append($"<br><b>Выключенные шаги:</b>");
