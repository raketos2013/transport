            ...
            : IStepService
{
    ...
    public async Task<CountFilesViewModel> CountFiles(int stepId)
    {
        var step = await GetStepByStepId(stepId)
                            ?? throw new DomainException("Шаг не найден");
        CountFilesViewModel countFiles = new();
        if (Directory.Exists(step.Source))
        {
            var files = Directory.GetFiles(step.Source, step.FileMask);
            countFiles.Count = files.Length;
            countFiles.IsExist = true;
        }
        else
        {
            countFiles.Count = 0;
            countFiles.IsExist = false;
        }
        return countFiles;
    }
