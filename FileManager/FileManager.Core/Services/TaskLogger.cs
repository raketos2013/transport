...
public class TaskLogger(IUnitOfWork unitOfWork) : ITaskLogger
{
    ...
    public async Task LogFiles(TaskStepEntity step, List<FileLog> files)
    {
        List<TaskLogEntity> logs = [];
        foreach (var fileLog in files)
        {
            logs.Add(new TaskLogEntity()
            {
                DateTimeLog = fileLog.DateTimeLog,
                OperationId = step.OperationId,
                ResultText = fileLog.Text,
                FileName = fileLog.FileName,
                StepId = step.StepId,
                TaskId = step.TaskId,
                StepNumber = step.StepNumber,
                OperationName = step.OperationName.ToString(),
                ResultOperation = fileLog.ResultOperation
            });
        }

        await unitOfWork.TaskLogRepository.AddTaskLogRange(logs);
        await unitOfWork.SaveAsync();
    }
